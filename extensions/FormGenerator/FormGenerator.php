 ...
 */
	...
	private function showRadio($data, $values, $selected) {		
		...
		//echo $sendValue;
		// Si no tiene múltiples valores (es un string), lo retornamos de golpe 
		if (!is_array($values)) {
		...
		// Por el contrario, tenemos 1 o más value con lo que es un posible texto
	...
	private function validateInput($data) {
		...
		if (isset($data['required']) and ($readValue === null or $readValue == '')) {
			$this->log(kw::te('is required', 'FormGenerator'), $data);
			return false;
		} elseif ($readValue == '') {
			return true;
		}
		...
		switch ($data['type']) {
			case 'text':
				...
				if (isset($data['maxlength']) and is_numeric($data['maxlength']) and $data['maxlength'] > -1 
					...
					$this->log(kw::te('is too long. Maximum', 'FormGenerator') . ' ' . $data['maxlength'] . ' ' . kw::te('characters', 'FormGenerator'), $data );
					return false;
				}
				...
				if (isset($data['pattern']) and is_numeric($data['pattern']) and $data['maxlength'] != '' 
					...
					$this->log(kw::te('pattern error', 'FormGenerator') . ' (' . $data['pattern'] . ')' , $data);
					return false;
				}
				break;
			case 'number':
			case 'range':
				...
				if ($readValue != '' and !is_numeric($readValue)) {
					$this->log(kw::te('Not numeric', 'FormGenerator'), $data);
					return false;
				}
				
				// MIN
				if (isset($data['min']) and $readValue < $data['min']) {
					$this->log(kw::te('The number have to be greather than', 'FormGenerator') . ' ' . $data['min'].'.', $data);
					return false;
				}
				
				// MAX
				if (isset($data['max']) and $readValue > $data['max']) {
					$this->log(kw::te('The number have to be less than', 'FormGenerator') . ' ' . $data['max'].'.', $data);
					return false;
				}
				...
				if (isset($data['step']) and $readValue != 0 and $readValue % $data['step'] !== 0) {
					$this->log(kw::te('The number have to be multiple of', 'FormGenerator') . ' ' . $data['step'].'.', $data);
					return false;
				}
			...
			case 'date';
				...
				if (!is_date($readValue)) {
					$this->log(kw::te('The date', 'FormGenerator') . ' ' .$readValue.' '. kw::te('must have format', 'FormGenerator') . ' mm/dd/yyyy '.'.', $data);
					return false;
				}
				break;
			...
			case 'email':
				if (!filter_var($readValue, FILTER_VALIDATE_EMAIL)) {
					$this->log(kw::te('Email invalid', 'FormGenerator'), $data);
					return false;
				}
			case 'file':
			...
			case 'url':
				if (!filter_var($readValue, FILTER_VALIDATE_URL)) {
					$this->log(kw::te('Invalid url', 'FormGenerator'), $data);
					return false;
				}
				break;	
		}
	...
	private function validateArray($values, $value, $data = array()) {
		if (is_array($values)) {
			...
			if (!array_key_exists($value, $values)) {
				if (is_array($value)) {
					$this->log(kw::te('Values don\'t match.', 'FormGenerator'), $data);
				} else {
					$this->log('ID ' .$value.' ' . kw::te('don\'t match', 'FormGenerator'), $data);
				}
				return false;
			}
		} else {
			// Is string
			if ($readValue == $values) {
				$this->log(kw::te('The value', 'FormGenerator') . ' ' . $value.' ' . kw::te('is not available', 'FormGenerator'), $data);
				return false;
			}
		}
	...
	private function log($message, $data = array()) {
		if ($this->logErrors) {
			...
			if (class_exists('kw')) {
				if (isset($data['label'])) $this->errors .= kw::te('Found an error in field', 'FormGenerator') . ': "' . $data['label'] . '": ';
				elseif (isset($data['name'])) $this->errors .= kw::te('Found an error in field', 'FormGenerator') . ': "' . $data['name'] . '": ';
			} else {
			...
			if (isset($data['title']) and $data['title'] != '') $this->errors .= ' | '. kw::te('MESSAGE', 'FormGenerator') .': ' . $data['title'];
